...
const App: React.FC = () => {
  ...
  // 動的なオフセット（ズレ）を管理する唯一のMotionValue
  ...
  const updateHomePositions = useCallback(() => {
    ...
    const trayTopY =
      (Math.floor(screenH / TILE_SIZE) - TRAY_GRID_ROWS) * TILE_SIZE;
    const trayLeftX =
      Math.floor((screenW - 10 * TILE_SIZE) / 2 / TILE_SIZE) * TILE_SIZE;
    ...
    setTiles((prev) =>
      prev.map((t) => {
        ...
        return {
          ...t,
          x: trayLeftX + c * TILE_SIZE,
          y: trayTopY + (r + 1) * TILE_SIZE,
          initialized: true,
        };
      })
    );
  ...
  const handleDragStart = (id: string) => {
    ...
    const targets = mainTile.groupId
      ? tiles.filter((t) => t.groupId === mainTile.groupId).map((t) => t.id)
      : [id];
  ...
  const handleDrag = useCallback(
    (id: string, info: PanInfo) => {
      ...
      setActiveDragInfo({
        snapDx,
        snapDy,
        isCollision: checkCollision(movingIds, snapDx, snapDy, tiles),
      });
    },
  ...
  const handleDragEnd = (id: string, info: PanInfo) => {
    ...
    // 吸着のアニメーション目標
    const targetX = isCollision ? 0 : snapDx;
    const targetY = isCollision ? 0 : snapDy;

    const config = { type: 'spring', stiffness: 1000, damping: 50, mass: 0.2 };
    setActiveDragId(null);
    setActiveDragInfo(null);

    // 全員同じMotionValueを使っているので一斉に動く
    Promise.all([
  ...
  const groupBoundaries = useMemo(() => {
    ...
    const existingGroups = Array.from(
      new Set(tiles.map((t) => t.groupId).filter(Boolean))
    );
    ...
    <div className="fixed inset-0 bg-white overflow-hidden touch-none">
      <div className="relative z-[100]">
        <Menu
          onReset={() =>
            setTiles((p) =>
              p.map((t) => ({ ...t, isPlaced: false, groupId: undefined }))
            )
          }
          onSave={() => alert('保存しました')}
          ...
          onGroupAction={() => {
            ...
            const groupsInSelection = new Set(
              selectedTiles.map((t) => t.groupId).filter(Boolean)
            );
            setTiles((prev) => {
              if (groupsInSelection.size > 0) {
                return prev.map((t) =>
                  groupsInSelection.has(t.groupId)
                    ? { ...t, groupId: undefined, version: t.version + 1 }
                    : t
                );
              }
              ...
              return prev.map((t) =>
                selectedIds.has(t.id)
                  ? { ...t, groupId: newGroupId, version: t.version + 1 }
                  : t
              );
            });
            setSelectedIds(new Set());
          ...
          hasGroupInSelection={Array.from(selectedIds).some(
            (id) => tiles.find((t) => t.id === id)?.groupId
          )}
        />
      </div>
      <Canvas />
      ...
      <div className="fixed inset-0 pointer-events-none z-50">
        {Object.entries(groupBoundaries).map(([gid, b]) => {
          const isMoving = movingIds.some(
            (id) => tiles.find((t) => t.id === id)?.groupId === gid
          );
              ...
              return (
                <div
                  ...
                  style={{
                    width: TILE_SIZE,
                    height: TILE_SIZE,
                    left: t.x + activeDragInfo.snapDx,
                    top: t.y + activeDragInfo.snapDy,
                  }}
                />
              );
            })}
        ...
        {tiles.map((tile) => (
          <div
            ...
            style={{ opacity: tile.initialized ? 1 : 0 }}
          >
            <DraggableTile
              tile={tile}
              ...
              groupColor={
                tile.groupId
                  ? GROUP_COLORS[groupBoundaries[tile.groupId]?.colorIndex]
                  : undefined
              }
              dragX={dragX}
              dragY={dragY}
